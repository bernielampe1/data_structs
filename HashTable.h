...
namespace bhl
{
  ...
  class HashTable
  {
    ...
    public:
      ...
      { for(unsigned i = 0; i < _size; i++) _array[i] = 0; }
      ...
      HashTable& operator=(const HashTable &rhs) { copy(rhs); return(*this); }
      ...
      void clear()
      {
        ...
        {
          Node *ptr = _array[i];
          while(ptr != 0) { Node *nx = ptr->_next; delete ptr; ptr = nx; }
          _array[i] = 0;
        }
      ...
      T& insert(const K &k, const T &d)
      {
        ...
        if (_array[b])
        {
          ...
          do
          {
            if (ptr->_key == k) { ptr->_data = d; return(ptr->_data); }

            if (ptr->_next == 0) break;
            else ptr = ptr->_next;
        ...
        if (exists(k))
        {
          ...
          while(ptr != 0)
          {
            if (ptr->_key == k)
            {
              if (ptr == _array[b])
              {
                if (ptr->_next) { _array[b] = ptr->_next; }
                else { _array[b] = 0; }

                delete ptr;
              }
              else if (ptr->_next == 0) { delete ptr; prev->_next = 0; }
              else { prev->_next = ptr->_next; delete ptr; }
      ...
      bool exists(const K &k) const
      {
        ...
        Node *ptr = _array[b];
        while(ptr != 0) { if (ptr->_key == k) return(true);
                          ptr = ptr->_next; }
      ...
      T& find(const K &k)
      {
        ...
        Node *ptr = _array[b];
        while(ptr != 0) { if (ptr->_key == k) { return(ptr->_data); }
                          ptr = ptr->_next; }
      }
