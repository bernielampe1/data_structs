...
namespace bhl
{
  ...
  class Queue
  {
    ...
    private:
      ...
      void copy(const Queue<T> &rhs)
      {
        if (this != &rhs)
        {
          ...
          {
             if (ptr2) ptr2 = ptr2->_next = new Node(ptr1->_data, 0, ptr2);
             else ptr2 = new Node(ptr1->_data, 0, 0);
    ...
    public:
      ...
      Queue& operator=(const Queue &rhs) { copy(rhs); return(*this); }
      ...
      {
        Node *ptr = _front;
        while(ptr != 0) { Node *tmp = ptr; ptr = ptr->_next; delete tmp; }
      ...
      void push(const T& d)
      {
        if (!empty()) _front = _front->_prev = new Node(d, _front, 0);
        else { _front = new Node(d, 0, 0); _back = _front; }
        _n++;
      }
      ...
      void pop()
      {
        if (!empty())
        {
          ...
          if (_n == 0) _front = _back = 0;
          if (_back) _back->_next = 0;
        }
      }
  ...
  std::ostream& operator<< (std::ostream &os, const Queue<T> &rhs)
  {
    ...
    while(ptr != 0) { os << ptr->_data << ", "; ptr = ptr->_prev;  }
