  ...
  class BinHeap
  {
    private:
      ...
      void _heapify(unsigned i)
      {
        ...
        if (l < size() && _data[l] CMPDIR _data[i]) t = l;
        if (r < size() && _data[r] CMPDIR _data[i]) t = r;
        if (t != i)
        {
    ...
    public:
      ...
      BinHeap(const BinHeap &rhs): _data(new T[rhs._n]), _n(rhs._n), _last(rhs._last)
      {
        for(unsigned i = 0; i < _n; i++) { _data[i] = rhs._data[i]; }
      }
      ...
      BinHeap& operator=(const BinHeap &rhs)
      {
        if (&rhs != this)
        {
          ...
          _n = rhs._n; _last = rhs._last _data = new T[_n];
