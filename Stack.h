...
namespace bhl
{
  ...
  class Stack
  {
    ...
    private:
      ...
      void copy(const Stack<T> &rhs)
      {
         if (this != &rhs)
         {
           ...
           {
             if (ptr2) ptr2 = ptr2->_prev = new Node(ptr1->_data, 0);
             else ptr2 = new Node(ptr1->_data, 0);
    ...
    public:
      ...
      Stack& operator=(const Stack &rhs) { copy(rhs); return(*this); }
      ...
      {
        Node *ptr = _top;
        while(ptr != 0) { Node *tmp = ptr; ptr = ptr->_prev; delete tmp; }
