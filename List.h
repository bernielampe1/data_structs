...
namespace bhl
{
  ...
  class List
  {
    ...
    public:
      class iterator
      {
        ...
        public:
          ...
          bool operator<(const iterator &rhs) const { return(_p < rhs._p); }
          bool operator>(const iterator &rhs) const { return(_p > rhs._p); }
          iterator& operator++() { _p++; _ptr = _ptr->_next; return(*this); }
          iterator operator++(int) { iterator it = *this; _p++; _ptr = _ptr->_next; return(it); }
          iterator& operator--() { _p--; _ptr = _ptr->_prev; return(*this); }
          iterator operator--(int) { iterator it = *this; _p--; _ptr = _ptr->_prev; return(it); }
      ...
      class const_iterator
      {
        ...
        public:
          ...
          bool operator<(const const_iterator &rhs) const { return(_p < rhs._p); }
          bool operator>(const const_iterator &rhs) const { return(_p > rhs._p); }
          const_iterator& operator++() { _p++; _ptr = _ptr->_next; return(*this); }
          const_iterator operator++(int) { const_iterator it = *this; _p++; _ptr = _ptr->_next; return(it); }
          const_iterator& operator--() { _p--; _ptr = _ptr->_prev; return(*this); }
          const_iterator operator--(int) { const_iterator it = *this; _p--; _ptr = _ptr->_prev; return(it); }
    ...
    public:
      ...
      {
        Node *ptr = _head;
        for(unsigned i = 0; i < _n; i++) { ptr = ptr->_next = new Node(0, ptr); }
        _tail = ptr;
      }

      List(const T &o, const unsigned n): _head(new Node(0, 0)), _tail(_head), _n(n)
      {
        Node *ptr = _head;
        for(unsigned i = 0; i < _n; i++) { ptr = ptr->_next = new Node(o, 0, ptr); }
        _tail = ptr;
      }

      List(const List &rhs): _head(new Node(0, 0)), _tail(_head), _n(rhs._n) { copy(rhs); }
      ...
      void resize(const unsigned n)
      {
        ...
        else if (n < _n)
        {
          ...
          { Node *ptr = _tail; _tail = _tail->_prev; delete ptr; }
          _tail->_next = 0;
          _n = n;
        }
      }
      ...
      void clear()
      {
        ...
        while(ptr != 0) { tmp = ptr; ptr = ptr->_next; delete tmp; }
        _tail = _head;
        _n = 0;
			}
      ...
      void pop_front()
      {
        if (!empty())
        {
          ...
          if (tmp->_next) tmp->_next->_prev = _head;
          delete tmp;
          _n--;
        }
      }
      ...
      iterator insert(const iterator &loc, const T &d)
      {
        ...
        if (tmp) tmp->_prev = loc._ptr->_next;

        if (loc._ptr == _tail) { _tail = loc._ptr->_next; }
        _n++;
      ...
      iterator erase(const iterator &loc)
      {
        if (!empty())
        {
          ...
          if (before) before->_next = after;
          if (after) after->_prev = before;
